...
sim_reg_stats(struct stat_sdb_t *sdb) {
    ...
    stat_reg_formula(sdb, "CPI_from_RAW_hazard_q2",
            ...
            "2 * one_cycle_q2 / sim_num_insn +3 * two_cycle_q2/sim_num_insn+1-sim_num_RAW_hazard_q2/sim_num_insn" /* ECE552 - MUST ADD YOUR FORMULA */, NULL);
        ...
        for (i = 0; i < 3; i++) {
            if (r_in[i] != DNA && reg_ready_q1[r_in[i]] > simucycle_q1) {
                ...
                if (reg_ready_q2[r_in[i]] - simucycle_q1 == 1) {
                    ...
                    fprintf(stderr, "fault: ** zhe shi wei shen me in q2  **\n");
                }
            }
        }
        sim_num_RAW_hazard_q2 = one_cycle_q2 + two_cycle_q2;
        if (r_out[0] != DNA){
            if((MD_OP_FLAGS(op) & F_MEM) &&(MD_OP_FLAGS(op) & F_LOAD))
                reg_ready_q2[r_out[0]] = simucycle_q2 + 3;
            else
                reg_ready_q2[r_out[0]] = simucycle_q2 + 2;}
        if (r_out[1] != DNA){
            if((MD_OP_FLAGS(op) & F_MEM) &&(MD_OP_FLAGS(op) & F_LOAD))
                reg_ready_q2[r_out[1]] = simucycle_q2 + 3;
            else
                reg_ready_q2[r_out[1]] = simucycle_q2 + 2;}
